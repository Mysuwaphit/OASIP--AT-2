...
const getEventList = async () => {
  ...
  if (res.status === 200) {
    ...
    // console.log(eventListDetails.value);
  } else {
    ...
    <div class="app" > 
        <div class="box" v-for="(categoryListDetail,index) in categoryListDetails" :key="index">
          ...
          <h1 id="bookingname">{{  categoryListDetail.eventCategoryName }}</h1>
                ...
                <p>Description :</p> 
                <p id="description" v-if="categoryListDetail.eventCategoryDescription != null">{{ categoryListDetail.eventCategoryDescription }}</p>
                <!-- event that in this category -->
                <div id="event" >
                  <p id="EventTitle" >All Event in {{ categoryListDetail.eventCategoryName }} : </p>
                    <ListEvent :events="eventListDetails" id="listevent" v-if="eventListDetails.length != 0"/>
