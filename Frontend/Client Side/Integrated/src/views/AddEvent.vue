...
const overlapping = () => {
  ...
  if(eventListByCategory.value.length != 0){
    ...
    const startDate = ref(getDate(e))
    const endDate = ref(getDate(e))
    const dateRange = moment.range(startDate.value, endDate.value)
    const validateDate  = (endDate.value == date) || (startDate.value == date)? true : false

    const startTime = ref(getTime(e))
    const endTime = ref(getEndTime(e))
    const timeRange = endTime.value < time || (startTime.value < time && time < endTime.value)? true : false

    console.log(`startTime : ${startTime.value}, endTime : ${endTime.value}`);
    console.log(`Range Date : ${dateRange} , Time : ${timeRange}`);
    // console.log(dateRange.contains(date))
    console.log(validateDate);
    console.log(timeRange);
    console.log(`${validateDate} : ${timeRange}` )
    return validate.value = validateDate && timeRange
  }))}
  console.log(`validate : ${validate.value}`);
  console.log(`Your Date : ${date} , Time : ${time}`);
  return validate.value === false? (validateDateTime.value = false) : alert('Please,Check your inputed Date&Time')
}
...
<template>
    <div class="app">
      ...
      <div id="boxAddEvent">
        ...
        <div class="modal-footer">
          <!-- :id=" validateDateTime === true? 'enable':'disable' " -->
    ...
    <SuccessBox v-if="success === true" class="trigger-btn" ref="#myModal" data-toggle="modal" />
</template> 
