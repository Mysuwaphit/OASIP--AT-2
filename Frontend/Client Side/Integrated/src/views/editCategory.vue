...
const updateEvent = async (categoryListDetailId,duration,description) => {
  console.log('success')
  const res = await fetch(`${import.meta.env.VITE_BASE_URL}/eventCategory/${categoryListDetailId}`,{
    method: 'PUT',
    headers: {
  ...
  // Match Event with param on URL
// const matchEvent = () => eventListDetails.value = eventListDetails.value.filter((event) =>  event.id.toString() === params.eventId)
            ...
            <h1 id="bookingname">{{ categoryListDetail.eventCategoryName }}</h1>
              <div id="detail">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="message-text"  maxlength="500" type="text" v-model="description" @vnode-before-mount="noteValue = categoryListDetail.eventCategoryDescription" >{{categoryListDetail.eventCategoryDescription}}</textarea> 
                </div>
                </form>
              </div>
              <!-- Footer -->
              <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary trigger-btn" id="save" href="#myModal" data-toggle="modal" @click="updateEvent(categoryListDetail.id,duration,description)">save</button>
